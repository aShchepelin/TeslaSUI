...
struct ClimateView: View {
    ...
    private var alertView: some View {
        VStack {
            if let url = URL(string: Constants.Text.teslaSupportLink) {
                ...
                Button(Constants.Text.closeButtonLabel) {
    ...
    private var acClimateSettingView: some View {
        HStack {
            ...
            Slider(value: $climateViewModel.circleProgress,
    ...
    private var fanClimateSettingView: some View {
        HStack {
            ...
            Slider(value: $climateViewModel.fanParameters,
                   in: Constants.Integer.sliderRange,
                   step: 1)
            .frame(width: 220)
        }
    ...
    private var autoClimateSettingView: some View {
        HStack {
            ...
            .neumorphismUnSelectedCircleStyle()
            Slider(value: $climateViewModel.autoParameters,
                   in: Constants.Integer.sliderRange,
                   step: 1)
            .frame(width: 220)
        }
    ...
    private var heatClimateSettingView: some View {
        HStack {
            ...
            Slider(value: $climateViewModel.heatParameters,
                   in: Constants.Integer.sliderRange,
                   step: 1)
            .frame(width: 220)
        }
    ...
    private var screenNameTextView: some View {
        Text(Constants.Text.climateScreenName)
            .font(.system(size: 20, weight: .semibold))
    }
    ...
    private var progressView: some View {
        ZStack {
            Circle()
                .fill(
                    LinearGradient(colors: [.ellipseTopGradient, .ellipseBottomGradient],
                                   startPoint: .topLeading,
                                   endPoint: .bottomTrailing))
                .frame(width: 168.2)
            ...
            Circle()
                .fill(
                    LinearGradient(colors: [.ellipseBottomGradient.opacity(0.4),
                                            .internalCircleColor],
                                   startPoint: .topLeading,
                                   endPoint: .bottomTrailing)
                )
    ...
    private func backgroundStackView<Content: View>(content: () -> Content) -> some View {
        ZStack {
            Rectangle()
                .fill(LinearGradient(colors: [Color.climateScreenTopGradient, Color.climateScreenBottomGradient],
                                     startPoint: .top,
                                     endPoint: .bottom))
   ...
   private func selectColorToGradient() -> LinearGradient {
            LinearGradient(colors: [climateViewModel.color, Color.bottomGradient],
                           startPoint: .leading,
                           endPoint: .trailing)
        }
}
